...
final class ExchangeUITests: XCTestCase {
    ...
    func testCalculatorScreenLoadsWithExpectedElements() throws {
        let screen = app.otherElements["exchangeListScreen"]
        XCTAssertTrue(screen.waitForExistence(timeout: 5))

        XCTAssertTrue(app.buttons["swapButton"].exists)
        XCTAssertTrue(app.textFields["amountField_USDC"].exists)
        XCTAssertTrue(app.textFields["amountField_MXN"].exists)
    }

    @MainActor
    func testSwapButtonFlipsCurrencies() throws {
        let topFieldBefore = app.textFields["amountField_USDC"]
        XCTAssertTrue(topFieldBefore.waitForExistence(timeout: 5))

        app.buttons["swapButton"].tap()

        let topFieldAfterSwap = app.textFields["amountField_MXN"]
        XCTAssertTrue(topFieldAfterSwap.waitForExistence(timeout: 3),
                       "After swap, MXN should be in the top position")

        let bottomFieldAfterSwap = app.textFields["amountField_USDC"]
    ...
    func testTypingAmountUpdatesOppositeField() throws {
        let topField = app.textFields["amountField_USDC"]
        XCTAssertTrue(topField.waitForExistence(timeout: 5))

        topField.tap()
        topField.clearAndTypeText("100")

        let bottomField = app.textFields["amountField_MXN"]
